...
export default function  from_addservices (props){
  const {check} = props.route.params;
    ...
    const addcategory = async() => {
        try {
            ...
            props.navigation.navigate('Ad_service',{
              check:1,
            })
            }
    ...
    const getcategories =async() => {
      try {
        const res = await fetch(`${baseurl}/api/services/${check}`);
    ...
    const updatecategories = async() => {
      try {
        if (name != "") {
          fetch(`${baseurl}/api/services/${check}`, {
          method: "PUT",
          headers: {
            Accept: "application/json",
            "Content-Type": "application/json",
