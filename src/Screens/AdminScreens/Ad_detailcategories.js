...
export default function Ad_detailcategories(props) {
  const { idcategories } = props.route.params;
  const [Categoriesdetail, setCategoriesdetail] = useState([]);
  const getcategories = async () => {
    try {
      const res = await fetch(`${baseurl}/api/categories/${idcategories}`);
  ...
  const deleteitem = async(id)=>{
    try {
        ...
        props.navigation.navigate('Ad_categories',{
          check:1,
        })
    } catch (error) {
      console.log(error);
  ...
  return (
    <SafeAreaView>
        ...
        >
            ...
            onPress={() => props.navigation.push('category_from',{check : Categoriesdetail.id})}
            >
